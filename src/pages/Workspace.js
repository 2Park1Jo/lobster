...
const Workspace = function () {
                ...
                if (departmentIdList.length !== stomp.counter - 3){
                    stomp.subscribe("/sub/chat/department/" + departmentIdList[departmentIdList.length - 1], function (chat) {
                        ...
                        if (result.contentType === "-1"){ // invite
                            setDpMemberUpdateState(Math.random());
                        }
        ...
        if (stomp.connected){
            ...
            stomp.subscribe("/sub/chat/workspace/" + localStorage.getItem('accessedWorkspaceId'), function (data) {
                if(data.body.includes("이미 존재하는 부서입니다")){
                    ...
                    setDepartmentUpdateState(Math.random());
            ...
            stomp.subscribe("/sub/chat/workspace", function (data) {
                ...
                setWorkspaceMemberUpdateState(Math.random());
                setDepartmentUpdateState(Math.random());
                setDpMemberUpdateState(Math.random());
            });
    ...
    function logout(){
        setLastChatData(
            ...
            chatViewModel.getLastChatData(localStorage.getItem('accessedDepartmentId')).chatId,
            chatViewModel.getChatLength(localStorage.getItem('accessedDepartmentId'))
        )
        localStorage.clear();
        stomp.disconnect()
        navigate('/')
